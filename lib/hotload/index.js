...
function read_file_failsafe(filename, cb)
{
  ...
  // 对于设置了watcher的变动的文件，重新从硬盘读取内容，并再次设置watcher
...
function extension_js(type, module, filename)
{
  ...
  is_hotswap_file = !!newmodule.hotload || newmodule && newmodule.exports && !!newmodule.exports.hotload;
  ...
  // json模块解析后默认绑定在module.exports属性中
...
function reload_file_force(filename, ntry)
{
  ...
  read_file_failsafe(filename, function (data) {
    ...
    clearCache.clearFileCache(filename);
...
{
  var w = fs.watch(filename, {persistent: false}, function(e) {
    if (autoreload) {
