    ...
    request: function( params, callback, errCallback, cookie ) {
        ...
        var req = http.request( options, function( res ) {
            ...
            res.on( 'end', function() {
                ...
                var tmp,result;
                try {
                    if(self._opt.encoding === ENCODING_RAW){
                        result = buffer;
                    }else{
                        tmp =  buffer.toString(self._opt.encoding);
                        result = ( self._opt.dataType !== 'json'
                          ? tmp
                          : JSON.parse( tmp ) );
                    }
                /*    result = self._opt.encoding === ENCODING_RAW
                        ...
                        //    : JSON.parse( (tmp = iconv.fromEncoding( buffer, self._opt.encoding ) ) ) );
                    ? (tmp =  buffer.toString(self._opt.encoding))
                : JSON.parse( (tmp =  buffer.toString(self._opt.encoding)) ) );*/
                } catch ( e ) {
                    clearTimeout( timer );
                    errCallback( new Error( "The result has syntax error. Hope to the " + self._opt.encoding + ', but parsed encounter an error. ' + tmp + ' ' + e ) );
                    return;
                }
                clearTimeout( timer );
            ...
            var req2 = http.request( options, function( res ) {
                ...
                res.on( 'end', function() {
                    ...
                    try {
                        ...
                        clearTimeout( timer );
                        errCallback( new Error( "Disaster Tolerant Request! The result has syntax error. Hope to the " + self._opt.encoding + ', but parsed encounter an error. ' + tmp + ' ' + e ) );
                        return;
                    }
                    clearTimeout( timer );
                    callback( result, res.headers['set-cookie'], res.headers);
