...
var MidProxy = require( path.join(process.cwd(),'/lib-cov/proxy/midproxy') );
var co = require('co');

describe( 'MidProxy', function() {
  describe( '#init()', function() {
    it( 'throws exception when the path of the interface is not right', function() {
      assert.throws( function() {
        MidProxy.init( 'error/path/interface_test.json' );
      }, function( err ) {
        return err.toString().indexOf( 'no such file or directory' ) !== -1;
      } );
    } );
  } );
  MidProxy.init( './tests/interface_test.json' );
  describe( '#create()', function() {
    it( 'should return an object with methods specified by the profile', function() {
      var m = MidProxy.create( 'Search.suggest' );
      assert( typeof m.suggest === 'function' );

      m = MidProxy.create( [ 'Search.suggest', 'Cart.getMyCart' ] );
      assert( typeof m.suggest === 'function' );
      ...
      m = MidProxy.create( 'Search.*' );
      assert( typeof m.list === 'function' );
      assert( typeof m.suggest === 'function' );
      assert( typeof m.getNav === 'function' );

      m = MidProxy.create( [ 'Search.getNav', 'D.getNav' ] );
    ...
    it('should throw exception if the specified interface id does not exist', function() {
      assert.throws( function() {
        var m = MidProxy.create( 'Search.getItems' );
      }, function( err ) {
...
describe( 'MidProxy', function() {
  ...
  describe( '#done()', function() {
    ...
    it( 'should write the data into request body when the method type is POST', function( done ) {
      var model = MidProxy.create( 'Test.post' );
      model.post( {
          a: 'abc',
          b: 'bcd',
          c: '{"a":"b"}'
      ...
      , function( done ) {
        ...
        co(function* (){
          ...
          var ret = yield new Promise(function(resolve,reject){
            m._done(resolve,reject);
          });
          assert( typeof ret[0] === 'object' );
          assert( typeof ret[1] === 'object' );
        });
      } );
